...
return function (args)
  ...
  local function push_opt(name, prefix)
    local value = prefix and {prefix, opt} or opt
    if opts[name] then
  ...
  for token in tokens do
    ...
    read_opt(token, '-r') and push_opt('rev') and push_opt('rev_date', '-r') or
    read_opt(token, '-D') and push_opt('date') and push_opt('rev_date', '-D') or
    push_file(token)
  end
  return files, opts
end
